...
if not GROQ_API_KEY:
    st.error(
        ...
        "Add it to `.env`, `.streamlit/secrets.toml`, or environment variables."
    )
    st.stop()
    ...
    with col2:
        ...
        if uploaded_file:
            ...
            if st.button(
                ...
                with st.spinner(
                    "Analyzing match... (30-60s)"
                ):

                    context = extract_resume_text(
                        uploaded_file
                    )
...
# ───────────────────────────────────────────────
    ...
    if (
        uploaded_file
        and st.session_state.resume_context is None
    ):
...
"""
        ...
        if prompt := st.chat_input(
            "Ask about career, resume, interviews..."
        ):
...
st.sidebar.caption(
    "**Pro Tips**: Use sidebar to switch tools instantly ⚡"
)
